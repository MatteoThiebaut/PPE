...
--
-- Hôte : 127.0.0.1
-- Généré le : lun. 16 nov. 2020 à 23:14
...
CREATE TABLE `commande` (
  ...
  `fk_id_pr` int(11) NOT NULL,
  `co_date` date DEFAULT NULL,
  `co_quantiter` int(11) DEFAULT NULL,
  `fk_en_id` int(11) DEFAULT NULL
...
INSERT INTO `commande` (`co_id`, `fk_id_pr`, `co_date`, `co_quantiter`, `fk_en_id`) VALUES
(13, 1, '2020-11-16', 975, 1),
(14, 5, '2020-11-16', 97, 2),
(15, 8, '2020-11-16', 96, 2);
...
IF new.fk_id_pr = (select fk_id_pr from commande where fk_id_pr=new.fk_id_pr) then 
SIGNAL SQLSTATE '45000'
SET MESSAGE_TEXT = 'la commande a deja etait faite !!';
end if;
END
$$
DELIMITER ;
...
CREATE TABLE `comprend` (
  ...
  `base_stock` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `comprend` (`fk_pr`, `fk_ce`, `fk_en`, `fk_ba`, `fk_mo`, `fk_ra`, `fk_se`, `fk_et`, `quantité`, `base_stock`) VALUES
(1, 1, 1, 1, 'M1', 'A', '1', '1', 25, 200),
(1, 2, 1, 6, 'M2', 'B', '12', '8', 25, 1000),
(3, 5, 3, 3, 'M5', 'J', '10', '6', 100, 100),
(3, 6, 3, 4, 'M6', 'L', '5', '3', 300, 300),
(5, 3, 2, 2, 'M3', 'C', '3', '4', 3, 100),
(8, 4, 2, 5, 'M4', 'F', '8', '2', 4, 100);
...
CREATE TABLE `entrepots` (
  ...
  `en_latitude` float DEFAULT NULL,
  `en_longitude` float DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `entrepots` (`en_id`, `en_nom`, `en_localisation`, `en_latitude`, `en_longitude`) VALUES
(1, 'A', 'Havre', 49.508, 0.144574),
(2, 'B', 'Marseille', 43.3004, 5.44779),
(3, 'C', 'Lyon', 45.7536, 4.85229);
...
INSERT INTO `login` (`fk_me`, `lo_password`) VALUES
('ppe', 'ppe');
...
INSERT INTO `membre` (`me_id`, `me_nom`, `me_prenom`, `me_email`, `me_sex`, `me_rang`) VALUES
('ppe', 'kycawazaky', 'alex', 'kkykawazaky@gmail.com', 1, 1);
...
ALTER TABLE `commande`
  ADD PRIMARY KEY (`co_id`,`fk_id_pr`),
  ADD KEY `fk_pr_idx` (`fk_id_pr`),
  ADD KEY `fk_en_id_commande_idx` (`fk_en_id`);
...
--
ALTER TABLE `commande`
  ADD CONSTRAINT `fk_en_id_commande` FOREIGN KEY (`fk_en_id`) REFERENCES `entrepots` (`en_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_pr_id_commande` FOREIGN KEY (`fk_id_pr`) REFERENCES `produits` (`pr_id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
